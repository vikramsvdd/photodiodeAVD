...
% correspondingly their power vector is 10^-6,10^-4,10^-2,10^0,
% this gets converted to a voltage vector after photodiode stage

if(miv==100)
    Voexp=[5.2e-4,0.04,0.7647,8.2]; % modelled according to non linear M v P
%characteristics,Mstinitial=100
elseif(miv==80)
    Voexp=[4.04e-4,0.0335,0.7292,7.97];%Mstinitial=80
elseif(miv==60)
    Voexp=[3.12e-4,0.0278,0.7276,8.19];%Mstinitial=60
elseif(miv==40)
    Voexp=[2.088e-4,0.0197,0.6816,8.14];%Mstinitial=40
elseif(miv==20)
    Voexp=[1.05e-4,0.0104,0.5675,8.03];%Mstinitial=20
end

In=[6.83e-4,0.066,6.66,666.6];
tc=[0,0,0,0];
...
noisyOutput=awgn(encodedInput,snr);
%}
...
legend('M_i=100','M_i=80','M_i=60','M_i=40','M_i=20','Location','southwest')
xlim([1 60])
ylim([0 1])
