...
if(miv==100)
    Voexp=[0.04,0.0688,0.0923,0.1127]; % modelled according to non linear M v P
%characteristics,Mstinitial=100
elseif(miv==80)
    Voexp=[0.0335,0.0592,0.0808,0.0998];%Mstinitial=80
elseif(miv==60)
    Voexp=[0.0278,0.0509,0.0711,0.0892];%Mstinitial=60
elseif(miv==40)
    Voexp=[0.0197,0.0375,0.0539,0.0691];%Mstinitial=40
elseif(miv==20)
    Voexp=[0.0104,0.0204,0.0302,0.0397];%Mstinitial=20
end
In=[0.066,0.132,0.198,0.264];
tc=[0,0,0,0];
...
%{ 
rough notes:
1. SER = 0.2481 for Pled = [1e-6, 6e-5, 2.5e-4, 9.5e-4]

2. Pled = [same for 3, 15.4e-4] M=32.8, Voexp=0.81, Vo=0.26, BER same
cannot get any worser

3.Pled =['', 5e-4] M=49.7, Voexp=0.26, Vo=0.13, still worse same fate
